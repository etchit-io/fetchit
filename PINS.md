...
| Asset                  | Pin                                  | Why                                                            |
|------------------------|--------------------------------------|----------------------------------------------------------------|
| `x0xd`                 | `0.34.3` (tail `1c6631ed78ee2b0c9111a034b701375ff6062075`) | Daemon REST + SSE + WS contract; bundled by desktop `build.rs` (`X0XD_PIN_VERSION`/`X0XD_PIN_SHA` — `check-pins.sh` asserts this row against those consts, so this row can no longer drift silently) and embedded on Android via the fetchit-ffi `x0x` git pin at the **same rev**. Post-defork: stock upstream v0.34.3 (which absorbed the 0.29-era actor-authz `committed_by`; it did NOT absorb returning-member re-key or `GET /groups/:id/secure/self` — verified empirically 2026-07-31) plus the short engine-A tail on `engine-a-34` (josh-clsn/x0x): relay-delivered group-join apply endpoints, TreeKEM join-retry convergence fixes, the returning-member re-key port (a81cbe1 → 5c52013), and the leaf-mode tail (`leaf_mode` config + vendored `saorsa-gossip-pubsub` pass-through patch — default OFF; only the Android embed opts in, and any build root embedding x0x must re-declare the `[patch.crates-io]` for `saorsa-gossip-pubsub` at the same rev or the embed fails to compile). G3 cross-NAT verified 2026-07-27. Defork runbook lives in the ops repo (`x0x-defork-plan`). |
