...
- Per-group **bridge consent** (default-off, per-group opt-in) — `crates/fetchit-chat/src/groups_reachability.rs:136` (`BridgeConsentStore`). In-memory on `chat`; encrypted at-rest persistence `[branch: bridge-consent-persist]` (487ba64/ab95505), pending cross-review + FF.
...
<!-- arch: id=chat-delivery glob=crates/fetchit-chat/src/conversation/outbound.rs crates/fetchit-chat/src/conversation/types.rs crates/fetchit-chat/src/groups_reachability.rs crates/fetchit-relay-server/src/transit.rs crates/fetchit-relay-server/src/ws.rs apps/fetchit-desktop/src/chat/outboxDriver.ts apps/fetchit-desktop/src/chat/bubble.ts verified=2eea972 -->
