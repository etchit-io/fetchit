...
/// Emit a single `WelcomeBlobRequest` envelope from the joiner to the
/// group owner.
///
/// Seals the request to the owner's KEM pubkey (from their stored
/// share-card). Returns [`ChatError::ShareCardMissing`] if the owner's
/// card has not yet been imported.
///
/// # Errors
/// - [`ChatError::ShareCardMissing`] when the owner's share-card is absent.
/// - [`ChatError::Invalid`] on KEM / AEAD / signing failure.
/// - [`ChatError::NoTransportAvailable`] / relay errors from [`Router::send`].
#[allow(clippy::too_many_arguments)]
pub async fn dispatch_welcome_request_to_owner<S>(
    signer: &S,
    router: &Router,
    layout: &StoreLayout,
    ...
    S: fetchit_relay_client::Signer + ?Sized,
{
    ...
    let owner_aid_hex = hex::encode(owner_agent_id);
    let kem_pub = recipient_kem_key(layout, &owner_aid_hex)?;

    let payload = WelcomeRequestPayload {
        group_id: group_id.to_owned(),
        joiner_agent_id: hex::encode(joiner_agent_id),
        ts_ms,
    };
    let parts = seal_welcome_request(&kem_pub, &payload)?;

    let local_agent_id = signer.agent_id();
    let mut env = TransitEnvelope {
        version: WIRE_VERSION,
        kind: EnvelopeKind::WelcomeBlobRequest,
        group_id: None,
        tenant_id: None,
        sender_agent_id: RelayAgentId::from_bytes(local_agent_id),
    ...
    let sig = signer
        .sign(&sign_bytes)
        .await
        .map_err(|e| ChatError::Invalid(format!("welcome-request envelope sign: {e}")))?;
    env.sender_signature = sig;

    let recipient = AgentId(hex::encode(owner_agent_id));
    let transport_out = OutboundEnvelope {
        kind: OutboundKind::Dm,
        from_machine_id: Some(local_machine_id),
        payload: Vec::new(),
...
/// Emit a single `WelcomeBlobResponse` envelope from the owner to the
/// joiner.
///
/// Seals the MLS Welcome blob bytes to the joiner's KEM pubkey (from
/// their stored share-card). Returns [`ChatError::ShareCardMissing`] if
/// the joiner's card has not yet been imported.
///
/// # Errors
/// - [`ChatError::ShareCardMissing`] when the joiner's share-card is absent.
/// - [`ChatError::Invalid`] on KEM / AEAD / signing failure.
/// - [`ChatError::NoTransportAvailable`] / relay errors from [`Router::send`].
#[allow(clippy::too_many_arguments)]
pub async fn dispatch_welcome_blob_to_joiner<S>(
    signer: &S,
    router: &Router,
    layout: &StoreLayout,
    joiner_agent_id: [u8; 32],
    ...
    S: fetchit_relay_client::Signer + ?Sized,
{
    ...
    let joiner_aid_hex = hex::encode(joiner_agent_id);
    let kem_pub = recipient_kem_key(layout, &joiner_aid_hex)?;
    ...
    let sig = signer
        .sign(&sign_bytes)
        .await
        .map_err(|e| ChatError::Invalid(format!("welcome-blob envelope sign: {e}")))?;
    env.sender_signature = sig;

    let recipient = AgentId(hex::encode(joiner_agent_id));
    let transport_out = OutboundEnvelope {
        kind: OutboundKind::Dm,
        from_machine_id: Some(local_machine_id),
        payload: Vec::new(),
...
mod tests {
    ...
    /// Capturing transport that records every `OutboundEnvelope` sent
    /// through it so dispatcher tests can inspect the wire envelopes
    /// without a real relay.
    struct CapturingTransport {
        sent: std::sync::Mutex<Vec<crate::transport::OutboundEnvelope>>,
    }
    ...
    impl CapturingTransport {
        ...
        fn captured(&self) -> Vec<crate::transport::OutboundEnvelope> {
    ...
    /// Joiner calls `dispatch_welcome_request_to_owner`; exactly one
    /// `WelcomeBlobRequest` envelope arrives at the capturing transport
    /// addressed to the owner, with the sealed payload unsealing to a
    /// `WelcomeRequestPayload` whose `group_id` matches.
    #[tokio::test]
    async fn dispatch_welcome_request_emits_one_envelope_to_owner() {
        use crate::groups::welcome_bridge::unseal_welcome_request;
        use std::sync::Arc;

        let dir = tempfile::tempdir().unwrap();
        let layout = StoreLayout::ensure(dir.path().to_path_buf()).unwrap();

        let owner_aid = [0xaau8; 32];
        let joiner_aid = [0xbbu8; 32];
        let machine_id = [0x01u8; 32];

        let (pk_owner, sk_owner) = kem_keygen().unwrap();
        store_card(&layout, owner_aid, &pk_owner);

        let capture = Arc::new(CapturingTransport::new());
        ...
        let sent = capture.captured();
        assert_eq!(sent.len(), 1, "exactly one envelope");

        let transit = sent[0].transit.as_ref().unwrap();
        assert_eq!(
            transit.kind,
            EnvelopeKind::WelcomeBlobRequest,
            "kind must be WelcomeBlobRequest"
        );
        ...
        .unwrap();
        assert_eq!(payload.group_id, "group-welcome-1");
        assert_eq!(payload.joiner_agent_id, hex::encode(joiner_aid));
        assert_eq!(payload.ts_ms, 42_000_u64);
    }

    /// Owner calls `dispatch_welcome_blob_to_joiner` with a 33 KiB blob;
    /// exactly one `WelcomeBlobResponse` envelope arrives at the capturing
    /// transport addressed to the joiner, with the sealed payload
    /// unsealing to a `WelcomeBlobPayload` whose `group_id` matches.
    #[tokio::test]
    async fn dispatch_welcome_blob_emits_one_envelope_to_joiner_with_33k_payload() {
        use crate::groups::welcome_bridge::unseal_welcome_blob;
        use base64::engine::general_purpose::STANDARD as B64;
        use base64::Engine as _;
        use std::sync::Arc;
        ...
        let joiner_aid = [0xbbu8; 32];
        let machine_id = [0x02u8; 32];

        let (pk_joiner, sk_joiner) = kem_keygen().unwrap();
        store_card(&layout, joiner_aid, &pk_joiner);

        let blob_bytes: Vec<u8> = vec![0xCDu8; 33 * 1024];
        ...
        assert_eq!(
            ...
            "kind must be WelcomeBlobResponse"
        );
        ...
        .unwrap();
        assert_eq!(payload.group_id, "group-welcome-1");
        assert_eq!(payload.ts_ms, 99_000_u64);
        assert_eq!(B64.decode(&payload.blob_b64).unwrap(), blob_bytes);
    }
