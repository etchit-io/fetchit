//! Shared wire schemas + the [`DenylistQuery`] trait for the fetch>it
//! trust service.
//!
//! This is a leaf crate (serde only) so consumers that need the trust
//! vocabulary, the stateless reader engine (`fetchit-core`), the chat
//! stack, and the consumer-side verifier, do not transitively compile
//! the trust *server* stack (axum, the HTTP client, the async runtime,
//! the PQ signer). The `fetchit-trust` server re-exports everything
//! here, so `fetchit_trust::EntryKind` and friends keep resolving.
