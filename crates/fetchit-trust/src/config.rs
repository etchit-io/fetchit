...
impl ServerConfig {
    ...
    /// `127.0.0.1:8090`), `FETCHIT_TRUST_DATA` (default
    /// `./fetchit-trust-data`), `FETCHIT_TRUST_ISSUER` (default
    /// `issuer-v1`).
    ///
    /// # Errors
    /// Returns `TrustError::Config` if `FETCHIT_TRUST_BIND` is malformed.
    pub fn from_env() -> Result<Self, TrustError> {
        ...
        Ok(Self::new(bind, data_dir, issuer_key_id))
    }
}
