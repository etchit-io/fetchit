#!/usr/bin/env sh
# Commit-aware staleness detector for the stamped docs (simple v1).
#
# Scans docs/ARCHITECTURE.md (structure) and docs/CAPABILITIES.md (what's built).
...
# the doc, the changed commits, and the fix: re-verify + bump the stamp).
#
...
docs="docs/ARCHITECTURE.md docs/CAPABILITIES.md"
found=0
for doc in $docs; do
  [ -f "$doc" ] && found=1
done
[ "$found" -eq 1 ] || { echo "ERROR: none of the stamped docs found ($docs)" >&2; exit 2; }
...
for doc in $docs; do
  ...
  printf '%s\n' "$stamps" | while IFS='	' read -r id sha glob; do
    ...
    if [ -n "$changed" ]; then
      {
        echo "ARCH-STALE: section '$id' ($doc) may be stale -- files under its glob ($glob) changed since $sha:"
        printf '%s\n' "$changed" | sed 's/^/    /'
...
[ "$any_stamps" -eq 1 ] || { echo "no arch stamps found in $docs" >&2; exit 0; }
